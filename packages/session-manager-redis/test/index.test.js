...
const redis = vi.hoisted(() => {
	...
	let subscription_delay;
	return {
		reset() {
			subscribers.clear();
			subscription_delay = undefined;
		},
		delaySubscription() {
			const started = Promise.withResolvers();
			const release = Promise.withResolvers();
			subscription_delay = { started, release };
		...
		createClient() {
			return {
				...
				async subscribe(channel, callback) {
					subscription_delay?.started.resolve();
					await subscription_delay?.release.promise;
...
describe('RedisSubscriptionManager', () => {
	...
	it('waits for the broker subscription before acknowledging', async () => {
		const delay = redis.delaySubscription();
