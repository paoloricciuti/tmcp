...
- **Release/worktree status**: Phase 3, its final review fixes, URL elicitation, the Phase 6 logging slice, HTTP coverage, stdio delivery, in-memory test synchronization, pnpm 11 CI alignment, and the sessionless in-memory client are committed through `4c0b6f8`. The current worktree completes Phase 4 subscriptions, including distributed adapters, the strict Phase 5.1 per-request HTTP runtime, and Phase 7 authorization conformance. `.changeset/warm-onions-repeat.md` covers the overall `2026-07-28` protocol feature and Phase 6 logging; `.changeset/mrtr-input-required.md` covers Phase 3 MRTR behavior and follow-up fixes; `.changeset/quiet-stdio-logs.md` covers immediate stdio request-notification delivery; `.changeset/tidy-stateless-clients.md` covers the in-memory client API; `.changeset/calm-streams-listen.md` covers subscriptions; `.changeset/distributed-subscriptions.md` covers the distributed subscription managers; `.changeset/strict-http-requests.md` covers strict HTTP behavior and its new core APIs; `.changeset/modern-auth-clients.md` covers Phase 7 authorization changes.
...
- **Done:** Redis, PostgreSQL, and Durable Objects `SubscriptionManager` adapters ship alongside their existing stream/info managers.

### 4.3 Distributed subscription managers — DONE

Each bundled distributed session-manager package now provides a separate implementation of the new contract:
...
The adapters distribute only notification publication. Each replica keeps its registrations and callbacks in a local `InMemorySubscriptionManager`, which provides filtering, acknowledgment buffering, FIFO delivery, typed request-ID identity, and closure. Redis uses one shared pub/sub channel. PostgreSQL uses LISTEN/NOTIFY with a temporary message row only for payloads that exceed the NOTIFY size limit. Durable Objects broadcast to connected manager sockets without storing subscription descriptors.

`HttpTransport` continues to default to `InMemorySubscriptionManager`; multi-replica deployments must explicitly pass the matching distributed manager through `subscriptionManager`.
...
- **Done:** add broker-only distributed implementations for PostgreSQL, Redis, and Durable Objects.
