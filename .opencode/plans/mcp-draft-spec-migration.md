...
- **Current verification**: the full workspace suite passes 320/320 tests across ten files plus 40/40 conformance checks, including 194 core, 36 HTTP transport, 35 in-memory transport, and one stdio transport test. Package TypeScript checks, ESLint, Prettier, generated declarations, publint, full workspace typecheck, and `git diff --check` all pass. The in-memory legacy elicitation tests now wait for asynchronous request preparation instead of assuming adapter conversion finishes within one microtask.
